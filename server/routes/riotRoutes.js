...
function addChamp(array, info) {
	...
	array[array.length] = {
		id: info.participants[0].championId,
		count: 1,
...
module.exports = function(router) {
	router.route('/riot/champion-by-summoners')
		...
		.get(function(req, res) {
			...
			getResource(url, function(championStats) {
				if (championStats === 'error') {
					...
					var mostPlayed = championStats.champions
						.sort(function(a, b) { 
			...
			async.each([url1, url2], function(url, summonersMining) {
				getResource(url, function(league) {
					if (league === 'error') {
						...
						var leagueEntries=league.entries
				...
				if (err) {
					...
					getResource(url, function(stats) {
						...
						res.status(200).send({
							base: playersBase,
							data: playersChampions,
							miningData: miningClasses,
							probabilityMe: me,
							result: result
						});
